...
- 用户于 2026-09-04 选择非确定 baseline 的 G0 方案 A，并批准把它写成 architectural specification clarification：同 horizon 定义 `d_B=D(B1,B2)`、`d_E=min(D(E,B1),D(E,B2))`，对每个已更新同形字段的 RMSE/MAE 分别要求 `d_E<=2*d_B`，`d_B=0` 时要求 exact；标量指标使用绝对差。结构、dtype/shape、Gaussian 数量、配置/输入/prior、legacy dispatch、RNG/相机轨迹、optimizer 结构/steps 和未激活字段仍是严格门。该规则不允许用 learned-output SHA/max-abs/mismatch count 单独否决，也不允许这些诊断量替代 RMSE/MAE。
...
- 方案 A 要求新的独立 8k baseline/baseline-repeat/E0 三次运行，在启动前冻结比较字段和运行协议，并使用 8k 自身的 baseline self-distance。它必须实际跨过 baseline 的 densify 600、trim 1000、Ray-Color 5001 与 ALR 7001 分支。若 8k 的 Gaussian 数量/shape 等严格门不一致或任一数值门超界，应报告 failure/coverage of baseline variability，不得对张量补齐、截断、重排或事后提高 2 倍系数。
