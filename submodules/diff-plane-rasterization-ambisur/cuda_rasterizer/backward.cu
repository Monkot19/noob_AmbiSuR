...
renderCUDA(
	...
	const uint32_t* __restrict__ n_contrib_first,
	...
	const bool disable_trunc)
{
	...
	const int first_contributor = inside ? n_contrib_first[pix_id] : 0;
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// float dchannel_dcolor_only_color;
            // if (contributor == first_contributor) {
            //     dchannel_dcolor_only_color = T; 
            // } else {
            //     dchannel_dcolor_only_color = alpha * T;
            // }
			// // END
...
void BACKWARD::render(
	...
	const uint32_t* n_contrib_first,
