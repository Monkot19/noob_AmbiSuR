...
**Temporal transition diagnostics clarification（2026-09-17，已批准）：** 上述“转移”严格指相邻 evidence refresh 的稳定状态时间转移 $s_i^{t-1}\to s_i^t$，不是同一刷新内的 candidate-to-stable 对照。每次刷新前，$s_i^{t-1}$ 必须已经通过现有 topology `new_to_old` 映射对齐到当前 Gaussian 域；survivor 保留历史，clone/split child 继承 mapped parent 历史，pruned row 离开当前域。严禁离线按相邻 snapshot 行号、最近邻或几何匹配近似身份。D0 维护以 refresh 为单位的连续稳定状态时长和 mapped-lineage 累积转移次数；每次刷新记录固定状态顺序下的 $5\times5$ count/fraction matrix、changed count/jitter rate、总体及逐状态 mean age/transition count。首次刷新以初始化 Bypass 为合法前态。现有 `.npz` 字段与 snapshot/checkpoint 行连接合同保持不变；摘要写入 `events.jsonl`，对齐诊断 tensor 随 versioned D0 runtime checkpoint 与 topology migration 保存。全部计算保持 `no_grad` 且仅供诊断，不得影响 gradient、optimizer、densification proxy、clone/split/prune/trim 或 lifecycle。缺少该 instrumentation 的首个 7k v1 run 只保留为只读工程证据，不得用相邻行近似补算或晋级；正式结果必须在新路径 v2 重跑。

**Topology identity/reset clarification（2026-09-17，已批准方案 A）：** `TopologyChange` 仍只保留一份 `new_to_old` 身份映射，但 `new_to_old` 与 `is_new` 不再互相等价。clone/split child 的 `new_to_old` 写入旧域 parent index，同时 `is_new=True`；真正没有 parent lineage 的新行才允许 `new_to_old=-1,is_new=True`。通用 Evidence/EMA/geometry-history/arbitration migration 继续以 `is_new` 为重置门，因此所有新 Gaussian 的 $A,S,T,V,K$ 与 geometry history 按既有合同清零；只有 temporal transition 的 age/count lineage migration 对 `new_to_old>=0` 的 clone/split child 继承 parent 历史。survivor 必须 `is_new=False,new_to_old>=0`，且任何 `new_to_old=-1,is_new=False` 非法。不得增加第二套 lineage map，也不得借该澄清让新点继承 Evidence、绕过 Probation 或改变 topology 动作。
