...
class GaussianModel:
    ...
    def densify_and_split(self, grads, grad_threshold, grads_abs, grad_abs_threshold, scene_extent, max_radii2D, N=2):
        ...
        # outlier_mask = noise.abs() > 2.0
        # while outlier_mask.any():
        #     noise[outlier_mask] = torch.randn_like(noise[outlier_mask])
        #     outlier_mask = noise.abs() > 2.0
        # samples = noise * stds
    ...
    def compute_weighted_sh_norm(self, weighting='equal'):
        ...
        if weighting == 'equal':
            ...
            # l=2 -> weight 6
            # l=3 -> weight 12
            total_energy = (2 * e_b1) + (6 * e_b2) + (12 * e_b3)
