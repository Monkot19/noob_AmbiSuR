...
**2026-09-15 多刷新工程门（PASS，非正式 D0）：** clean `research/core-routing@66c1ef1f8708fa14fc422477759a5cf680054132` 的 Tool Room r2/seed0/500 iteration run 完成 100/200/300/400/500 五次 refresh；exit 0、272 s、峰值 4,896 MiB、0 error/nonfinite/GT，200,000 Gaussian 且未到首次 densification 600。`V_g` 从首次的 0 在 iteration 200 恢复至 0.99842，随后保持约 0.997–0.999；`V_pg` 约 0.997–0.999，双方可靠条件下 joint coverage 约 0.99989–1.0，joint-invalid 每次仅 0–1 行。逐 Gaussian 重放 `H_enter=3` 的 stable mismatch 全为 0，四段 invalid `T_p/T_g/K` EMA history mismatch 全为 0；final checkpoint refresh count/last=`5/500`、10 个快照映射字段 mismatch 全为 0、optimizer step=499。工程 runtime/EMA/hysteresis/checkpoint 门已满足；但仓库尚无 GT offline label/evaluator，设计稿也未冻结每 Gaussian GT 距离与 high-error label 定义，因此正式 7k/G1 在该 architectural clarification 前继续停止。

**Offline G1 architectural clarification（已批准，2026-09-16）：** 用户批准综合方案 A，并已正式同步至设计稿 G1。每 Gaussian 原始 GT 几何误差固定为其中心到对齐 GT mesh 三角面的最近无符号欧氏距离，使用世界坐标米制和 float64 分块 exact point-to-triangle query，不按 scaling、opacity 或 scene extent 归一化。主要 high-error 标签固定为 `e_i>0.05 m`；2 cm、10 cm 和场景内 top-20% 仅作预注册敏感性分析，不参与 G1 PASS。若主要标签阳性率低于 5% 或高于 95%，G1 记为不可评估并停止，不得事后切换阈值。G1 评价 checkpoint 中全部有限 Gaussian 中心，并对完整有效 GT mesh 的全部有限、非退化三角面求最近距离；不按 opacity、scaling、可见次数、视锥、AABB 或人工 crop 过滤。若 GT mesh 存在覆盖或对齐问题，立即停止并报告，不临时修改评价域。GT 量只允许由独立 offline evaluator 生成，不得进入训练、evidence snapshot/cache 或 checkpoint。正式 D0 每 1000 iteration 写 no-GT snapshot，保存 iteration 3000/7000 checkpoint；7000 是唯一 G1 PASS 主门，3000 只作预注册 early diagnostic，禁止跨时间挑最好结果。可视化采用自动确定性方案：3000/7000 输出 A/S/N/Tp/Tg/K/state/GT-distance 彩色 PLY 与固定三视角 PNG，7000 输出主 ROC/PR/risk-coverage/状态误差图，1000–7000 输出状态/转移/joint-coverage 时序；视角按 COLMAP image ID 排序的 25%/50%/75%，全画幅无 crop，色域和 palette 固定，输出 SVG/PDF/CSV/JSON，坏视角不得替换。产物与训练目录隔离，并打包包含全部图片、PLY 和表格的 `.tar.gz` 加 SHA256，完成后回报绝对服务器地址供下载。下一步仅编写 TDD 实施计划和 evaluator；正式 7k 仍需实现与 exploratory smoke 通过后单独授权。
...
- [x] 提交/推送 test+fix+三份规划记录：`457c965796b752057935cd68d2f740c2bbf32ba4`；不改训练、GT、评价域或阈值。
...
- [ ] Task 4 loader 子步进入 GREEN 验证：test-only `6fac88dd3b62241d712a84d3f68d80534f8b18d5` 已在 AutoDL 精确观察到缺少 `reliability.g1_timeline` 的预期 RED；本地最小 loader 候选只逐轮读取七个 schema-2 event/snapshot，禁止相邻 snapshot 行对齐，等待服务器 focused GREEN 后再进入三个 timeline bundle。
