...
| 2026-09-03 | 首次 paired-500 baseline launcher 的数据安全门强制要求原始 `sparse/0/*.bin`，在 `cameras.bin` 处停止 | 1 | 训练未启动、方法结果未产生；代码证据显示原始 pose loader 允许 `cameras/images.{bin|txt}`，而 aligned prior 才硬性使用 `points3D.bin`。先只读列出服务器实际 COLMAP 文件，再按真实格式收窄安全门；不得据此要求重传数据 |
