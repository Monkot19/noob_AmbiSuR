...
2. 历史 schema-2 G0 的 343 项内部 summary FAIL 原样保留；按后来批准且预先冻结的 behavioral schema-3 合同，独立 unseen E0 正式审计已经 `g0_equivalent=true`，hard exact/numeric failures `0/0`，因此 Phase 1 的 behavioral G0 门已通过。346 项内部 outlier 仍是诊断证据，不解释为内部轨迹一致。用户已经授权 D0 TDD；当前 D0 Steps 1–8 均已在 AutoDL 通过 focused/full 组件回归门，150-iteration/单刷新真实 shadow 工程 smoke 及隔离审计也已通过。当前阻塞转为多刷新（建议 500 iteration/interval 100）工程门、正式 Tool Room 7k D0、离线 G1 与 D0 对 baseline 的正式等价检查；C1、Supporting、阶段 tag 仍未获准。
...
5. 用户回传的最新服务器执行提交为 clean `research/core-routing@3a54a881193701759ca9c55d6e70384d627f302c`；真实 shadow smoke exit 0，唯一 iteration-100 refresh、快照/checkpoint/input/Git 深审通过。当前本地仅同步验证记录；不修改 baseline/结果资产，不创建 tag，也不把该短步工程 smoke 记为正式 D0/G1 或授权 C1。
