...
5. 用户回传的最新服务器执行提交为 clean `research/core-routing@66c1ef1f8708fa14fc422477759a5cf680054132`；500-iteration 五刷新 shadow smoke 的 runtime、coverage、hysteresis、EMA isolation、checkpoint 和 Git 深审通过。当前本地仅同步验证记录并进入 offline evaluator architectural clarification；不修改 baseline/结果资产，不创建 tag，也不把工程 smoke 记为正式 D0/G1 或授权 C1。
