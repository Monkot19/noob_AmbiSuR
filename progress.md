...
2. Tool Room 旧 8k E0 自身门通过，但正式 schema-2 G0 因 343 项内部 numerical summary 失败而保持历史 `G0=FAIL`。behavioral schema-3 合同已确认；空 app Tensor、exploratory CLI、正式合同/输入不可变性组件在 AutoDL `301e697` 上 full 78/78 PASS，旧报告逐字节回放亦通过。当前关键未完成项是把已批准规格和证据文档提交/同步，冻结新 E0 的 pre-launch 合同，运行独立 unseen E0 8k 并正式审计。新确认通过前 D0/C1 暂停，factor 固定 `2.0`。
...
5. 用户回传的服务器复核位于 clean `research/core-routing@301e69781861c32e61100251e5ebe150940a4e7c`；正式合同 targeted/full 测试后 clean。本地仅保留尚未同步的已批准规格、implementation spec/plan 与三份证据规划文档；训练/方法源码、结果资产和 tag 未改变。
